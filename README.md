...
| Gledopto RGB+CCT LED Controller | 1 | ZigBee | [deCONZ](https://www.home-assistant.io/integrations/deconz/) | Compared to the OSRAM strips this is bad. I'm using the controller with cheap bulk LED strip so I don't now *why* this is bad but I have not heard good things aboud Gledopto so maybe it really is the controller. The LEDs take a hell of a long time to turn on and switch colors, the warmer whites are not nice to look at (I'm using a orangey red instead) and the light just goes out completely when going below 30% brightness. |

### Presence Detection

| Device | Quantity | Connectivity | Integration | Notes |
| -------| :------: | ------------ | ----------- | ----- |
| Aqara Human Motion Sensor | 4 | ZigBee | [deCONZ](https://www.home-assistant.io/integrations/deconz/) | These are pretty great. I order them exclusively from Hong Kong so they are also cheap. The motion and brightness detection work like a charm but the temperature sensors on all my devices register values slightly off. Best thing is how small and unobstrusive they are.
